module leeminhduc2::simple_trade_example {
    ...
    fun init_module(admin : &signer) {
        ...
        let admin_cap = resource_account::retrieve_resource_account_cap(admin, @leeminhduc2);
    ...
    fun get_resource_signer() : signer acquires ModuleData {
        let module_data = borrow_global_mut<ModuleData>(@leeminhduc2_rsrc_account);
    ...
    #[view]
    public fun get_metadata(user : address, name : vector<u8>) : Object<Metadata> {
        let asset_address = object::create_object_address(&user, name);
    ...
    fun mint(name : vector<u8>, to : address, amount: u64) acquires ManagedFungibleAsset, ModuleData {
        let asset = get_metadata(@leeminhduc2, name);
    ...
    public entry fun transfer(from: &signer, to: address, name : vector<u8>, amount: u64) {
        ...
        let asset = get_metadata(from_address, name);
    ...
    fun test_trade(user1 : &signer, user2 : &signer) acquires ModuleData, ManagedFungibleAsset {
        ...
        let metadata1 = get_metadata(user1_address, user1_coin_name);
        let metadata2 = get_metadata(user2_address, user2_coin_name);
